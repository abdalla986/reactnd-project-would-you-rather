...
class Leaderboard extends Component {
  render() {
    ...
    const orderedUsers = users.sort((a, b) => b.totalScore - a.totalScore)
...
function mapStateToProps({ users }) {
  const usersSet = Object.values(users)

  usersSet.map((user) => user.totalScore = Object.keys(user.answers).length + user.questions.length)
